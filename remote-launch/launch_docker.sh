...
if docker ps --filter "name=$1" --filter "status=running" | grep -q $1; then
    sudo docker exec $1 /bin/bash -c "source ~/.bashrc && ros2 launch holohover_utils $2 experiment:='$3' machine:='$4' record:='true'"
else
    ...
    --network host \
    $1 bash -c "source /root/.bashrc && ros2 launch holohover_utils $2 experiment:='$3' machine:='$4' record:='true'"
fi